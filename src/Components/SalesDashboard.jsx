...
const CustomTick = (props) => {
  ...
  return <text x={x} y={y} textAnchor="middle" fill="#666">{payload.value}</text>;
};
              ...
              adminBagRef
                ...
                .then((bagSnapshot) => {
                  ...
                  bagSnapshot.forEach((bagDoc) => {
                    ...
                    const adminRef = firebase.firestore().collection('Admins').doc(bagData.uid);
                    const promise = adminRef.get().then((adminDoc) => {
                      if (adminDoc.exists) {
                        const adminName = adminDoc.data().name;
                        console.log('Admin Name:', adminName); // Log the admin name
                        const adminIndex = adminCommissionsData.findIndex(
                          (item) => item.id === bagData.uid
                        );
                        if (adminIndex !== -1) {
                          adminCommissionsData[adminIndex].name = adminName;
                        }
                      }
                    });
